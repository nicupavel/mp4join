...
// HARDCODED ! (variable framerate doesn't work right)
double VideoFrameRate = 30.0;
u_int32_t Mp4TimeScale = 90000;
bool TimeScaleSpecified = false;
bool VideoProfileLevelSpecified = false;
int VideoProfileLevel;


typedef struct _mpeg4_frame_t mpeg4_frame_t;
struct _mpeg4_frame_t{
  mpeg4_frame_t *next;
		       ...
		       bool allowVariableFrameRate)
{
    ...
    fprintf(stderr, "Video profile: %d\n", videoProfileLevel);
    ...
    while (LoadNextObject(inFile, pObj, &objSize, &objType)) {
        ...
        if (objType == MP4AV_MPEG4_VOSH_START) {
            ...
            MP4AV_Mpeg4ParseVol(pObj, objSize,
                    ...
                    &frameWidth, &frameHeight, NULL, NULL, NULL);
    ...
    if (VideoFrameRate) {
      ...
      fprintf(stderr,"***** TimeScale: %d, Frame Rate: %lf, Frame duration: %d\n", Mp4TimeScale, VideoFrameRate, mp4FrameDuration); 
    ...
    fprintf(stderr,"es config size is %d\n", esConfigSize);
    if (esConfigSize) {
